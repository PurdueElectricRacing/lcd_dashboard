...
typedef struct
{
	uint16_t avg_volts_bms;
	uint16_t low_volts_bms;
	uint16_t max_volts_bms;
	uint16_t avg_temp_bms;
	uint16_t high_temp_bms;
	uint16_t low_temp_bms;
	int16_t pack_current_bms;
	uint16_t instant_pack_volt_bms;
	int16_t pack_current_low_bms;
	uint16_t pack_disch_lim_bms;
	uint16_t soc_cell_lim_bms;
	uint8_t car_state;
	uint8_t PC_status;
	uint8_t tctv_state;
	uint32_t wheel_speed_front_l;
	uint32_t wheel_speed_front_r;
	uint32_t wheel_speed_rear_l;
	uint32_t wheel_speed_rear_r;
	uint8_t proportional_gain;
	uint8_t integral_gain;
	uint8_t derivative_gain;
	uint8_t signal_deadzone;
}data_t;
typedef struct
{
...
#endif /* LCD_H_ */
>>>>>>> Stashed changes:lcd_steering/Inc/lcd.h
