...
//    //receive can messages and update the lcd screen as necessary
//    //Live SOC/Voltage/Temperature
	if (xQueueReceive(lcd.q_rx_can, &rx_can, TIMEOUT) == pdTRUE)
	{
		switch (rx_can.StdId)
		{
			case AVG_CELL_VOLTAGE_ID:
	...
	if (new_integral_gain != integral_gain ||
			new_proportional_gain != proportional_gain||
			new_derivative_gain != derivative_gain ||
			new_signal_deadzone != signal_deadzone)   //Checks if new values match the old values. If not, then it updates the values.
	{
		integral_gain = new_integral_gain;
		proportional_gain = new_proportional_gain;
		derivative_gain = new_derivative_gain;
		...
		tx.RTR = CAN_RTR_DATA;
		tx.DLC = 4;
		tx.Data[0] = (uint8_t) (lcd.data.proportional_gain);
		tx.Data[1] = (uint8_t) (lcd.data.proportional_gain >> 8);
		tx.Data[2] = (uint8_t) (lcd.data.integral_gain);
		tx.Data[3] = (uint8_t) (lcd.data.integral_gain >> 8);
		xQueueSendToBack(lcd.q_tx_can, &tx, 100);
		tx.StdId = ID_UPDATE_PARAM_2;
		tx.Data[0] = (uint8_t) (lcd.data.derivative_gain);
		tx.Data[1] = (uint8_t) (lcd.data.derivative_gain >> 8);
		tx.Data[2] = (uint8_t) (lcd.data.signal_deadzone);
		tx.Data[3] = (uint8_t) (lcd.data.signal_deadzone >> 8);
		xQueueSendToBack(lcd.q_tx_can, &tx, 100);
	}
//    if (xQueuePeek(lcd.q_rx_can, &rx_can, TIMEOUT) == pdTRUE)
//    {
...
>>>>>>> Stashed changes:lcd_steering/Src/lcd.c
